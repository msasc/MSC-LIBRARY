...
/**
 * JSON parser. Parses standard ECMA-404 types plus extended types.
 * 
 * @author Miquel Sas
 */
public class JSONParser {
	...
	private JSONObject parseObject() throws IOException {
		...
		while (true) {
			...
			if (token.type != JSONTypes.STRING) throw new IOException(fmtErr);
			...
			if (token.kind == Kind.VALUE) {
				obj.put(key, token.type, token.value);
				...
				if (token.value.equals("{")) {
					...
					if (entry_ext != null)
						...
						obj.put(key, JSONTypes.OBJECT, nextObj);
				}
				/* Start of a JSON array. */
				if (token.value.equals("[")) {
					JSONArray arr = parseArray();
					obj.put(key, JSONTypes.ARRAY, arr);
				}
			}
	...
	private JSONArray parseArray() throws IOException {
		...
		while (true) {
			...
			/* Token is a value. */
			if (token.kind == Kind.VALUE) {
				arr.add(token.type, token.value);
			}
			...
			if (token.kind == Kind.STRUCT) {
				...
				if (token.value.equals("{")) {
					...
					if (entry_ext != null)
						...
						arr.add(JSONTypes.OBJECT, nextObj);
				}
				/* Start of a JSON array. */
				if (token.value.equals("[")) {
					JSONArray nextArr = parseArray();
					arr.add(JSONTypes.ARRAY, nextArr);
				}
			}
	...
	private JSONEntry getExtendedTypeEntry(JSONObject obj) {
		...
		JSONTypes type = null;
		for (JSONTypes t : JSONTypes.extendedTypes) {
		...
		JSONTypes type_obj = entry_obj.getType();
		...
		if (type_obj == JSONTypes.NULL) return new JSONEntry(type, null);
		...
		if (type == JSONTypes.BINARY) {
			...
			return new JSONEntry(type, bytes);
		}
		...
		if (type == JSONTypes.DATE) {
			...
			return new JSONEntry(type, date);
		}
		...
		if (type == JSONTypes.TIME) {
			...
			return new JSONEntry(type, time);
		}
		...
		if (type == JSONTypes.TIMESTAMP) {
			...
			return new JSONEntry(type, timestamp);
		}

		/* Type decimal, let new BigDecimal(string) do the job. */
		...
		/* Type double, let Double.valueOf do the job. */
		...
		/* Type integer, let Integer.valueOf do the job. */
		...
		/* Type long, let Long.valueOf do the job. */
			...
			while (true) {
				...
				if (":,]}".indexOf(c) >= 0) {
					try {
						...
						nextToken = new Token(Kind.STRUCT, JSONTypes.STRING, String.valueOf((char) c));
