...
public class DBConnectionPool {
	...
	private class ConnectionCloser extends TimerTask {
		...
		public void run() { closeConnections(); }
	}
	...
	/**
	 * A boolean that indicates that this database engine has been closed and no
	 * more connections can be requested.
	 */
	private boolean closed = false;

	/**
	 * Constructor.
	 * @param cnInf The connection information.
	 */
	...
	public void close() throws SQLException {
		try {
			lock.lock();
			timer.cancel();
			for (DBConnection cn : connectionsInUse) { cn.closeUnderlyingConnection(); }
			connectionsInUse.clear();
			for (DBConnection cn : connectionsFree) { cn.closeUnderlyingConnection(); }
	...
	public int getCloserExecutions() { return closerExecutions; }
	public int getRemovedConnections() { return removedConnections; }
	...
	public DBConnection getConnection() throws SQLException {
		try {
			lock.lock();
			if (closed) throw new SQLException("DBEngine is closed");
