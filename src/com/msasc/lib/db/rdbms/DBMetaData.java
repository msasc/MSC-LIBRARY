...
public abstract class DBMetaData {
	 ...
	 * @throws SQLException If an error occurs.
	 */
	public abstract RecordSet getTables(String catalog, String schema, String table) throws SQLException;
	...
	protected RecordList readRecordSet(DBResultSet rs, FieldList fields) throws SQLException {
		...
		while (rs.next()) {
			...
			for (Field field : fields) {
				ValueReader reader = (ValueReader) field.getProperties().getObject(ValueReader.KEY);
				if (reader != null) {
					reader.read(rs, record, field);
				} else {
					Value value;
					if (field.isPersistent() || field.isVirtual()) {
						value = db.fromResultSet(rs, field);
					} else {
